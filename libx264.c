...
typedef struct X264Context {
    ...
    /* ffmetric features from final x264 statistics */
    int ffmetric_i_count;
    int ffmetric_p_count;
    int ffmetric_b_count;

    double ffmetric_i_avg_qp;
    double ffmetric_p_avg_qp;
    double ffmetric_b_avg_qp;

    int ffmetric_i_size;
    int ffmetric_p_size;
    int ffmetric_b_size;

    double ffmetric_consecutive_bframes[4];

    double ffmetric_mb_i[3];

    double ffmetric_mb_p_i[3];
    double ffmetric_mb_p_p[5];
    double ffmetric_mb_p_skip;

    double ffmetric_mb_b_i[3];
    double ffmetric_mb_b_b[3];
    double ffmetric_mb_b_direct;
    double ffmetric_mb_b_skip;
    double ffmetric_mb_b_l0;
    double ffmetric_mb_b_l1;
    double ffmetric_mb_b_bi;

    double ffmetric_transform_8x8_intra;
    double ffmetric_transform_8x8_inter;

    double ffmetric_coded_intra[3];
    double ffmetric_coded_inter[3];

    double ffmetric_i16[4];
    double ffmetric_i8[9];
    double ffmetric_i4[9];
    double ffmetric_i8c[4];

    double ffmetric_weighted_p_y;
    double ffmetric_weighted_p_uv;

    double ffmetric_ref_p_l0[16];
    double ffmetric_ref_b_l0[16];
    double ffmetric_ref_b_l1[16];

    int ffmetric_ref_p_l0_count;
    int ffmetric_ref_b_l0_count;
    int ffmetric_ref_b_l1_count;

    double ffmetric_output_bitrate_kbps;

    int ffmetric_has_i;
    int ffmetric_has_p;
    int ffmetric_has_b;

    int ffmetric_x264_rc_is_crf;
    double ffmetric_x264_crf;
} X264Context;
...
static void ffmetric_fill_features(const X264Context *x4, double *f)
{
    f[0]  = x4->ffmetric_output_bitrate_kbps;     // output_bitrate_kbps

    f[1]  = x4->ffmetric_i_count;                 // i_frame_count
    f[2]  = x4->ffmetric_i_avg_qp;                // i_frame_avg_qp
    f[3]  = x4->ffmetric_i_size;                  // i_frame_avg_size_bytes

    f[4]  = x4->ffmetric_p_count;                 // p_frame_count
    f[5]  = x4->ffmetric_p_avg_qp;                // p_frame_avg_qp
    f[6]  = x4->ffmetric_p_size;                  // p_frame_avg_size_bytes

    f[7]  = x4->ffmetric_b_count;                 // b_frame_count
    f[8]  = x4->ffmetric_b_avg_qp;                // b_frame_avg_qp
    f[9]  = x4->ffmetric_b_size;                  // b_frame_avg_size_bytes

    f[10] = x4->ffmetric_consecutive_bframes[0];  // consec_b_frames_0B_pct
    f[11] = x4->ffmetric_consecutive_bframes[1];  // consec_b_frames_1B_pct
    f[12] = x4->ffmetric_consecutive_bframes[2];  // consec_b_frames_2B_pct
    f[13] = x4->ffmetric_consecutive_bframes[3];  // consec_b_frames_3B_pct

    f[14] = x4->ffmetric_mb_i[0];                 // mb_I_I16_pct
    f[15] = x4->ffmetric_mb_i[1];                 // mb_I_I8_pct
    f[16] = x4->ffmetric_mb_i[2];                 // mb_I_I4_pct

    f[17] = x4->ffmetric_mb_p_i[0];               // mb_P_I16_pct
    f[18] = x4->ffmetric_mb_p_i[1];               // mb_P_I8_pct
    f[19] = x4->ffmetric_mb_p_i[2];               // mb_P_I4_pct

    f[20] = x4->ffmetric_mb_p_p[0];               // mb_P_P16_pct
    f[21] = x4->ffmetric_mb_p_p[1];               // mb_P_P8_pct
    f[22] = x4->ffmetric_mb_p_p[2];               // mb_P_P4_pct
    f[23] = x4->ffmetric_mb_p_skip;               // mb_P_skip_pct

    f[24] = x4->ffmetric_mb_b_i[0];               // mb_B_I16_pct
    f[25] = x4->ffmetric_mb_b_i[1];               // mb_B_I8_pct
    f[26] = x4->ffmetric_mb_b_i[2];               // mb_B_I4_pct

    f[27] = x4->ffmetric_mb_b_b[0];               // mb_B_B16_pct
    f[28] = x4->ffmetric_mb_b_b[1];               // mb_B_B8_pct
    f[29] = x4->ffmetric_mb_b_b[2];               // mb_B_B4_pct

    f[30] = x4->ffmetric_mb_b_direct;             // mb_B_direct_pct
    f[31] = x4->ffmetric_mb_b_skip;               // mb_B_skip_pct
    f[32] = x4->ffmetric_mb_b_l0;                 // mb_B_L0_pct
    f[33] = x4->ffmetric_mb_b_l1;                 // mb_B_L1_pct
    f[34] = x4->ffmetric_mb_b_bi;                 // mb_B_BI_pct

    f[35] = x4->ffmetric_transform_8x8_intra;     // transform_8x8_intra_pct
    f[36] = x4->ffmetric_transform_8x8_inter;     // transform_8x8_inter_pct

    f[37] = x4->ffmetric_coded_intra[0];          // coded_y_intra_pct
    f[38] = x4->ffmetric_coded_intra[1];          // coded_uvDC_intra_pct
    f[39] = x4->ffmetric_coded_intra[2];          // coded_uvAC_intra_pct

    f[40] = x4->ffmetric_coded_inter[0];          // coded_y_inter_pct
    f[41] = x4->ffmetric_coded_inter[1];          // coded_uvDC_inter_pct
    f[42] = x4->ffmetric_coded_inter[2];          // coded_uvAC_inter_pct

    f[43] = x4->ffmetric_i16[0];                  // i16_v_pct
    f[44] = x4->ffmetric_i16[1];                  // i16_h_pct
    f[45] = x4->ffmetric_i16[2];                  // i16_dc_pct
    f[46] = x4->ffmetric_i16[3];                  // i16_p_pct

    f[47] = x4->ffmetric_i8[0];                   // i8_v_pct
    f[48] = x4->ffmetric_i8[1];                   // i8_h_pct
    f[49] = x4->ffmetric_i8[2];                   // i8_dc_pct
    f[50] = x4->ffmetric_i8[3];                   // i8_ddl_pct
    f[51] = x4->ffmetric_i8[4];                   // i8_ddr_pct
    f[52] = x4->ffmetric_i8[5];                   // i8_vr_pct
    f[53] = x4->ffmetric_i8[6];                   // i8_hd_pct
    f[54] = x4->ffmetric_i8[7];                   // i8_vl_pct
    f[55] = x4->ffmetric_i8[8];                   // i8_hu_pct

    f[56] = x4->ffmetric_i4[0];                   // i4_v_pct
    f[57] = x4->ffmetric_i4[1];                   // i4_h_pct
    f[58] = x4->ffmetric_i4[2];                   // i4_dc_pct
    f[59] = x4->ffmetric_i4[3];                   // i4_ddl_pct
    f[60] = x4->ffmetric_i4[4];                   // i4_ddr_pct
    f[61] = x4->ffmetric_i4[5];                   // i4_vr_pct
    f[62] = x4->ffmetric_i4[6];                   // i4_hd_pct
    f[63] = x4->ffmetric_i4[7];                   // i4_vl_pct
    f[64] = x4->ffmetric_i4[8];                   // i4_hu_pct

    f[65] = x4->ffmetric_i8c[0];                  // i8c_dc_pct
    f[66] = x4->ffmetric_i8c[1];                  // i8c_h_pct
    f[67] = x4->ffmetric_i8c[2];                  // i8c_v_pct
    f[68] = x4->ffmetric_i8c[3];                  // i8c_p_pct

    f[69] = x4->ffmetric_weighted_p_y;            // weighted_pframe_Y_pct
    f[70] = x4->ffmetric_weighted_p_uv;           // weighted_pframe_UV_pct

    f[71] = x4->ffmetric_ref_p_l0[0];             // ref_P_L0_r0_pct
    f[72] = x4->ffmetric_ref_p_l0[1];             // ref_P_L0_r1_pct
    f[73] = x4->ffmetric_ref_p_l0[2];             // ref_P_L0_r2_pct
    f[74] = x4->ffmetric_ref_p_l0[3];             // ref_P_L0_r3_pct

    f[75] = x4->ffmetric_ref_b_l0[0];             // ref_B_L0_r0_pct
    f[76] = x4->ffmetric_ref_b_l0[1];             // ref_B_L0_r1_pct
    f[77] = x4->ffmetric_ref_b_l0[2];             // ref_B_L0_r2_pct

    f[78] = x4->ffmetric_ref_b_l1[0];             // ref_B_L1_r0_pct
    f[79] = x4->ffmetric_ref_b_l1[1];             // ref_B_L1_r1_pct
}

static double ffmetric_predict_xgboost(const X264Context *x4)
{
    double f[VQM4HAS_XGB_N_FEATURES];

    ffmetric_fill_features(x4, f);
...
static void X264_log(void *p, int level, const char *fmt, va_list args)
{
    ...
    if (x4 && sscanf(line, "frame %c:%d%*[^0-9.]%lf%*[^0-9]%d",
                 &frame_type, &count, &avg_qp, &size) == 4) {
        switch (frame_type) {
        case 'I':
            x4->ffmetric_i_count = count;
            x4->ffmetric_i_avg_qp = avg_qp;
            x4->ffmetric_i_size = size;
            x4->ffmetric_has_i = 1;
            break;
        case 'P':
            x4->ffmetric_p_count = count;
            x4->ffmetric_p_avg_qp = avg_qp;
            x4->ffmetric_p_size = size;
            x4->ffmetric_has_p = 1;
            break;
        case 'B':
            x4->ffmetric_b_count = count;
            x4->ffmetric_b_avg_qp = avg_qp;
            x4->ffmetric_b_size = size;
            x4->ffmetric_has_b = 1;
            break;
        }
    } else if (x4 && !strncmp(line, "consecutive B-frames:", 21)) {
        ffmetric_parse_percent_list(line + 21, x4->ffmetric_consecutive_bframes, 4);

    } else if (x4 && !strncmp(line, "mb I", 4)) {
    const char *i16 = strstr(line, "I16..4:");
        if (i16)
            ffmetric_parse_percent_list(i16 + strlen("I16..4:"),
                                    x4->ffmetric_mb_i, 3);

    } else if (x4 && !strncmp(line, "mb P", 4)) {
        const char *i16 = strstr(line, "I16..4:");
        ...
        if (i16)
            ffmetric_parse_percent_list(i16 + strlen("I16..4:"),
                                    x4->ffmetric_mb_p_i, 3);

        if (p16)
            ffmetric_parse_percent_list(p16 + strlen("P16..4:"),
                                    x4->ffmetric_mb_p_p, 5);

        if (skip)
            sscanf(skip, "skip: %lf%%", &x4->ffmetric_mb_p_skip);

    } else if (x4 && !strncmp(line, "mb B", 4)) {
        const char *i16 = strstr(line, "I16..4:");
        ...
        if (i16)
            ffmetric_parse_percent_list(i16 + strlen("I16..4:"),
                                    x4->ffmetric_mb_b_i, 3);

        if (b16)
            ffmetric_parse_percent_list(b16 + strlen("B16..8:"),
                                    x4->ffmetric_mb_b_b, 3);

        if (direct)
            sscanf(direct, "direct: %lf%%", &x4->ffmetric_mb_b_direct);

        if (skip)
            sscanf(skip, "skip: %lf%%", &x4->ffmetric_mb_b_skip);

        if (l0)
            sscanf(l0, "L0: %lf%%", &x4->ffmetric_mb_b_l0);

        if (l1)
            sscanf(l1, "L1: %lf%%", &x4->ffmetric_mb_b_l1);

        if (bi)
            sscanf(bi, "BI: %lf%%", &x4->ffmetric_mb_b_bi);

    } else if (x4 && !strncmp(line, "8x8 transform", 13)) {
        sscanf(line, "8x8 transform intra:%lf%% inter:%lf%%",
               &x4->ffmetric_transform_8x8_intra,
               &x4->ffmetric_transform_8x8_inter);

    } else if (x4 && !strncmp(line, "coded y,uvDC,uvAC", 17)) {
        sscanf(line,
               "coded y,uvDC,uvAC intra: %lf%% %lf%% %lf%% inter: %lf%% %lf%% %lf%%",
               &x4->ffmetric_coded_intra[0],
               &x4->ffmetric_coded_intra[1],
               &x4->ffmetric_coded_intra[2],
               &x4->ffmetric_coded_inter[0],
               &x4->ffmetric_coded_inter[1],
               &x4->ffmetric_coded_inter[2]);

    } else if (x4 && !strncmp(line, "i16 v,h,dc,p:", 13)) {
        ffmetric_parse_percent_list(line + 13, x4->ffmetric_i16, 4);

   } else if (x4 && !strncmp(line, "i8 ", 3)) {
        const char *p = strchr(line, ':');

        if (p)
            ffmetric_parse_percent_list(p + 1, x4->ffmetric_i8, 9);

    } else if (x4 && !strncmp(line, "i4 ", 3)) {
        const char *p = strchr(line, ':');

        if (p)
            ffmetric_parse_percent_list(p + 1, x4->ffmetric_i4, 9);

    } else if (x4 && !strncmp(line, "i8c dc,h,v,p:", 13)) {
        ffmetric_parse_percent_list(line + 13, x4->ffmetric_i8c, 4);

    } else if (x4 && !strncmp(line, "Weighted P-Frames:", 18)) {
        sscanf(line, "Weighted P-Frames: Y:%lf%% UV:%lf%%",
               &x4->ffmetric_weighted_p_y,
               &x4->ffmetric_weighted_p_uv);

    } else if (x4 && !strncmp(line, "ref P L0:", 9)) {
        x4->ffmetric_ref_p_l0_count =
            ffmetric_parse_percent_list(line + 9, x4->ffmetric_ref_p_l0, 16);

    } else if (x4 && !strncmp(line, "ref B L0:", 9)) {
        x4->ffmetric_ref_b_l0_count =
            ffmetric_parse_percent_list(line + 9, x4->ffmetric_ref_b_l0, 16);

    } else if (x4 && !strncmp(line, "ref B L1:", 9)) {
        x4->ffmetric_ref_b_l1_count =
            ffmetric_parse_percent_list(line + 9, x4->ffmetric_ref_b_l1, 16);

    } else if (x4 && !strncmp(line, "kb/s:", 5)) {
        sscanf(line, "kb/s:%lf", &x4->ffmetric_output_bitrate_kbps);
    }
...
static av_cold int X264_close(AVCodecContext *avctx)
{
    ...
    if (x4->ffmetric_has_i && x4->ffmetric_has_p & x4->ffmetric) {
        double ffmetric_xgboost = ffmetric_predict_xgboost(x4);
        if (ffmetric_xgboost < 0.0)
            ffmetric_xgboost = 0.0;
        else if (ffmetric_xgboost > 100.0)
            ffmetric_xgboost = 100.0;
        av_log(avctx, AV_LOG_INFO, "FFmetric:%.2f\n", ffmetric_xgboost);
    }
...
static const AVOption options[] = {
    ...
    { "ffmetric", "Enable FFmetric perceptual quality prediction", OFFSET(ffmetric), AV_OPT_TYPE_BOOL, { .i64 = 0 },0,1,VE },
    { NULL },
};
